...
const RouteDetailsModal = ({ route, onClose, onUpdate }) => {
  ...
  // Helper: normalize bin status from whatever shape the backend uses.
  const normalizeBinStatus = (binInfo) => {
    // possible fields: binInfo.status, binInfo.collectStatus, binInfo.collected (boolean)
    const s = (binInfo.status || binInfo.collectStatus || (binInfo.collected === true ? 'completed' : undefined) || '').toString().toLowerCase();
  ...
  // Sort bins by sequenceOrder / order if present
  const binsSorted = useMemo(() => {
    if (!route.wasteBins) return [];
    return [...route.wasteBins].sort((a, b) => {
      const aOrder = (a.sequenceOrder ?? a.order ?? a.sequence ?? 0);
      const bOrder = (b.sequenceOrder ?? b.order ?? b.sequence ?? 0);
      return aOrder - bOrder;
    });
  }, [route.wasteBins]);

  // Determine which bin should be labelled 'next' — first pending by order
  const nextIndex = useMemo(() => {
    for (let i = 0; i < binsSorted.length; i++) {
      const st = normalizeBinStatus(binsSorted[i]);
      if (st === 'pending') return i;
      if (st === 'next') return i; // if backend explicitly marks it
    }
    return -1;
  }, [binsSorted]);

  const getBinBadge = (status, isNext) => {
    if (isNext) return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">Next</span>;
    switch (status) {
      case 'completed':
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div className="relative top-10 mx-auto p-5 border w-full max-w-4xl shadow-lg rounded-md bg-white">
        ...
        {/* Show list of bin addresses with status behind each address */}

{binsSorted && binsSorted.length > 0 && (
  <div className="mt-6">
    <h4 className="text-md font-medium text-gray-900 mb-3">Waste Bins ({binsSorted.length})</h4>
    <div className="space-y-2 max-h-72 overflow-y-auto">
      {binsSorted.map((binInfo, idx) => {
        const status = binInfo.status;
        const binID = binInfo.bin.binId;
        const address = binInfo.bin?.location?.address || binInfo.location?.address || 'Address not available';
        const seq = binInfo.sequenceOrder ?? binInfo.order ?? binInfo.sequence ?? idx + 1;

        return (
          <div key={binInfo.bin?._id || `${seq}-${idx}`} className="flex items-center justify-between p-3 border border-gray-200 rounded-md">
            <div>
              <div className="text-sm font-medium">
                {seq}. {user?.userType === 'admin' ? `${address} (ID: ${binID})` : address}
              </div>
              {binInfo.priority && (
                <div className="text-xs text-gray-400">Priority: {binInfo.priority}</div>
              )}
            </div>

            <div className="ml-4">{getBinBadge(status)}</div>
          </div>
        );
      })}
    </div>
